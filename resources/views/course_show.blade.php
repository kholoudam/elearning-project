    ...
    <div class="container my-5">
        <div class="card shadow">
            <div class="card-body">
                ...
                <p class="card-text"><strong>Catégorie :</strong> {{ $cours->categorie }}</p>
                <p class="card-text"><strong>Difficulté :</strong> {{ $cours->difficulte }}</p>
                @if($cours->enseignant && $cours->enseignant->role === 'enseignant')
                    <p class="card-text"><strong>Enseignant :</strong> {{ $cours->enseignant->nom }}</p>
                @else
                    <p class="card-text"><strong>Enseignant :</strong> Non défini ou rôle invalide</p>
                @endif
            </div>
            <div class="card-footer text-end">
                <a href="{{ route('courses.index') }}" class="btn btn-secondary">Retour aux cours</a>
                @auth
                    @php
                        $user = auth()->user();
                        $isInscrit = App\Models\Inscription::where('utilisateur_id', $user->id)->where('cours_id', $cours->id)->exists();
                    @endphp
                    @if($user->role === 'etudiant')
                        @if(!$isInscrit)
                            <form action="{{ route('cours.inscrire', $cours->id) }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-success ms-2" style="background-color: rgba(24, 56, 24, 0.7);">
                                    S'inscrire à ce cours
                                </button>
                            </form>
                        @else
                            <a href="{{ route('cours.apprendre', $cours->id) }}" class="btn btn-outline-custom ms-2">Accéder au cours</a>
                        @endif
                    @endif
                @else
                    <a href="{{ route('login.show') }}" class="btn btn-success ms-2" style="background-color: rgba(24, 56, 24, 0.7);">
